...
export default function SpriteSidebar({ pokemonNames }: SpriteSidebarProps) {
  ...
  return (
    <div className="w-full h-full bg-(--background-secondary) border-l border-white/10 flex flex-col items-center py-6 gap-2 overflow-y-auto animate-fade-in custom-scrollbar transition-all duration-300">
      <h3 className="text-xs font-bold text-(--text-secondary) uppercase tracking-wider mb-2 text-center">
        Team Sprites
      </h3>
      ...
      {loading ? (
        <div className="flex flex-col gap-4">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="w-16 h-16 rounded-full bg-white/5 animate-pulse"
            />
          ))}
        </div>
      ) : (
